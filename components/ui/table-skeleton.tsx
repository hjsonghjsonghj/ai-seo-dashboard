      ...
      <div className="overflow-hidden rounded-lg border border-border-secondary/50">
        <div className="grid" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
          {Array.from({ length: cols }).map((_, i) => (
            <div key={`h-${i}`} className="h-10 animate-pulse border-b border-border-secondary/50 bg-surface-hover/55" />
          ))}
          {Array.from({ length: rows * cols }).map((_, i) => (
            <div
              key={`c-${i}`}
              className="h-12 animate-pulse border-b border-border-secondary/30 bg-surface-default/45"
            />
          ))}
        </div>
      </div>
